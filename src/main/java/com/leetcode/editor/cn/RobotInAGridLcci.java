...
public class RobotInAGridLcci {
    ...
    class Solution {
        ...
        private boolean dfs(int[][] obstacleGrid, int i, int j, List<List<Integer>> ret) {
            ...
            if (i < length - 1 && dfs(obstacleGrid, i + 1, j, ret) ||
                    j < width - 1 && dfs(obstacleGrid, i, j + 1, ret)) {
                List<Integer> cur = new ArrayList<>();
                cur.add(i);
                cur.add(j);
                ret.add(0, cur);
